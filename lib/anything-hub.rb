...
module AnythingHub
  ...
  def run(input, opts = {})
    ...
    if (results = command(input))
      cache_store.write(input, results.to_json)
  ...
  def token
    unless (_token = cache_store.read('authorizations:token') || config.token)
      res = `curl \
        --data '{"scopes":["repo"]}' \
      ...
      cache_store.write 'authorizations:token', _token
    end
    @token ||= _token
  end

  def cache_store
    @cache_store ||= ActiveSupport::Cache::FileStore.new(File.join(app_dir, 'cache'))
  end
end
